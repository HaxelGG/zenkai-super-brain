...
- **Local live testing:** create a gitignored `web/.env` with `ZENKAI_API_KEY` (+ `N8N_WEBHOOK_URL`, `ANTHROPIC_API_KEY`, `ELEVENLABS_API_KEY` as available); `astro dev` loads it. Astro requires a dev-server restart to pick up `.env` changes.
