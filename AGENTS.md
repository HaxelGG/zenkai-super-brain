...
- **Pending (need design + provider keys):** deeper agent orchestration and any Cursor-style agentic features. Add each as its own `web/src/lib/*` module + `/api/*` route + mocked test, gated behind its own env var.
